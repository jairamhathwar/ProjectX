...
class TrajTrackingDyn(TrajTrackingBase):
    ...
    def __init__(self, Tf, N, params_file='modelparams.yaml', base_path=""):
        """
    ...
    def define_sys_dyn(self):
        ...
        # slip angle
        alpha_f = delta - casadi.if_else(Vx >= 0.5,
                                         casadi.atan2(
                                             (omega * l_f + Vy), Vx), 0)
        alpha_r = casadi.if_else(Vx >= 0.5, casadi.atan2((omega * l_r - Vy),
                                                         Vx), 0)
        # alpha_f = delta - casadi.atan2((omega * l_f + Vy), Vx)
        # alpha_r = casadi.atan2((omega * l_r - Vy), Vx)

        # Tire force
        F_x = (casadi.if_else(d >= 0, (C_m1 - C_m2 * Vx), C_m2 * Vx) * d -
               C_roll - C_d * Vx * Vx) / 2
        F_x = casadi.fmin(F_x, 5)
        # F_fy = casadi.if_else(Vx>=1, D_f*casadi.sin(C_f*casadi.atan(B_f*alpha_f)), 0)
        # F_ry = casadi.if_else(Vx>=1, D_r*casadi.sin(C_r*casadi.atan(B_r*alpha_r)), 0)
        F_fy = D_f * casadi.sin(C_f * casadi.atan(B_f * alpha_f))
        F_ry = D_r * casadi.sin(C_r * casadi.atan(B_r * alpha_r))

        # state vector
        x = casadi.vertcat(pos_X, pos_Y, Vx, Vy, psi, omega)
        x_dot = casadi.vertcat(pos_X_dot, pos_Y_dot, Vx_dot, Vy_dot, psi_dot,
                               omega_dot)

        # control input
        u = casadi.vertcat(d, delta)

        # system dynamics
        f_expl = casadi.vertcat(
            Vx * casadi.cos(psi) - Vy * casadi.sin(psi),  # X_dot
            Vx * casadi.sin(psi) + Vy * casadi.cos(psi),  # Y_dot
            (F_x + F_x * casadi.cos(delta) - F_fy * casadi.sin(delta) +
             m * Vy * omega) / m,  # Vx_dot
            (F_ry + F_x * casadi.sin(delta) + F_fy * casadi.cos(delta) -
             m * Vy * omega) / m,  # Vy_dot
            omega,
            ((F_fy * casadi.cos(delta) + F_x * casadi.sin(delta)) * l_f -
             F_ry * l_r) / Iz)
    ...
    def define_constrint(self):
        ...
        deltadot_min = self.params[
            'deltadot_min']  # minimum steering angle cahgne[rad/s]
        deltadot_max = self.params[
            'deltadot_max']  # maximum steering angle cahgne[rad/s]

        self.ocp.constraints.idxbu = np.array([0, 1])
        self.ocp.constraints.lbu = np.array([d_min, delta_min])
        self.ocp.constraints.ubu = np.array([d_max, delta_max])
    ...
    def define_cost(self):
        ...
        p = casadi.vertcat(pos_X_ref, pos_Y_ref, psi_ref, vel_ref)

        self.acados_model.p = p

        delta_pos_x = self.acados_model.x[0] - pos_X_ref
        delta_pos_y = self.acados_model.x[1] - pos_Y_ref
        delta_psi = casadi.mod((self.acados_model.x[4] - psi_ref + np.pi / 2),
                               2 * np.pi) - np.pi / 2
...
if __name__ == '__main__':
    ...
    ''' turn and slow down '''
    # dt = 1.0/step
    # dangle = np.linspace(0, np.pi/10/N, N)
    ...
    ref_traj = np.array(
        [[2.17068132, 2.16921533, 2.15857991, 2.12655402, 2.06254802],
         [
             0.02421046,
             0.06713086,
             0.14433611,
             ...
             1.94606698,
             2.09760154,
         ], [0.25158538, 0.59795171, 0.76903226, 1.11447322, 1.47407364]])

    x_0 = np.array([
        2.17277314e+00, 1.70393955e-02, 2.32692650e-03, -7.69616184e-04,
        1.59578767e+00, -6.24971426e-03
    ])
    ...
    ocp = TrajTrackingDyn(
        ...
        N,
        params_file=
        '/home/nvidia/Documents/PrincetonRaceCar/ROS_Package/src/Control/traj_tracking_ros/params/modelparams.yaml'
    )
    import time
    start = time.time()
